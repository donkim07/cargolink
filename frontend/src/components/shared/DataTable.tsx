...
export function DataTable<T>({ columns, data, className, minWidth = '640px' }: DataTableProps<T>) {
            ...
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id} className="border-b border-forest/10 bg-forest/5">
                {hg.headers.map((header) => (
                  <th key={header.id} className="whitespace-nowrap px-4 py-3 text-left font-semibold text-charcoal/70">
